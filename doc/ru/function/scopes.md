...
Нередко можно столкнуться с неприятным последствием единого глобального пространства имён — проблема с перекрытием имён переменных. В JavaScript эту проблему легко избежать, используя *анонимные обёртки*.
...
Безымянные функции являются отложенными [выражениями](#function.general) (_прим. перев._ — то есть они не разбираются по месту описания, а откладываются парсером напоследок); поэтому, чтобы сделать их исполняемыми, следует спровоцировать их разбор.
