...
> **Замечание:** Наличие параметра функции с именем `arguments` **запрещает** движку создать объект `arguments`, создающийся, в других случаях, по умолчанию.
...
Безымянные функции являются отложенными [выражениями](#function.general) (_прим. перев._ — то есть они не выполняются по месту описания, а откладываются движком напоследок); поэтому, чтобы сделать их исполняемыми, сначала следует спровоцировать их разбор.
